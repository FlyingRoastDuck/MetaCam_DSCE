...
from reid.trainers import Noisy_USL
from reid.evaluators import Evaluator, extract_features
from reid.utils.logging import Logger
from reid.utils.serialization import load_checkpoint, save_checkpoint
from reid.utils.faiss_rerank import compute_jaccard_distance
from reid.utils.tools import get_test_loader, get_train_loader
...
def main_worker(args):
    ...
    # Trainer
    trainer = Noisy_USL(model, memory)
    ...
    for i, ((fname, _, cid), label) in enumerate(zip(sorted(dataset.train), pseudo_labels)):
        pseudo_labeled_dataset.append((fname, label.item(), i))
    # The same as ECN
    ...
    for epoch in range(args.epochs):
        ...
        for i, ((fname, _, cid), label) in enumerate(zip(sorted(dataset.train), pseudo_labels)):
            # ignore -1 samples
            if label.item() != -1:
                # take camera to represent index
                pseudo_labeled_dataset.append((fname, label.item(), i))
        # statistics of clusters and un-clustered instances
        memory.module.labels = torch.from_numpy(pseudo_labels).cuda()
        train_loader = get_train_loader(dataset.images_dir, args.height, args.width,
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="MetaCam without outliers")
    # data
